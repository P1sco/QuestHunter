...
def get_template_path():
    """
    Looks for Binaries/Win64/main.mfs in two places:
    1. Outside, directly relative to where QuestHunter.exe sits.
    2. Inside, extracted into PyInstaller's temporary sys._MEIPASS directory.
    """
    exe_dir = os.path.dirname(os.path.abspath(sys.argv[0]))
    external_template = os.path.join(
        exe_dir, "Binaries", "Win64", "main.mfs"
    )
    ...
    if getattr(sys, "frozen", False):
        internal_template = os.path.join(
            sys._MEIPASS, "Binaries", "Win64", "main.mfs"
        )
        if os.path.exists(internal_template):
...
class IconDownloader(QThread):
    ...
    def run(self):
        ...
        file_name = f"{self.game_id}_{os.path.basename(self.icon_url)}"
...
class GameCard(QFrame):
    ...
    def __init__(self, game_id, game_name, icon_url, exec_path="", parent=None):
    ...
    def load_icon(self, icon_url):
        if icon_url and icon_url.startswith(("http://", "https://")):
            file_name = f"{self.game_id}_{os.path.basename(icon_url)}"
            local_path = os.path.join(CACHE_DIR, file_name)
...
class LauncherWindow(QMainWindow):
    ...
    def __init__(self):
        ...
        self.status_label = QLabel("Select a game to launch")
    ...
    def on_json_loaded(self, games_list):
        ...
        for game in sorted_games:
            ...
            card = GameCard(g_id, name, icon_url, exec_path)
            self.all_cards.append(card)

        self.rebuild_grid(self.all_cards)
        self.status_label.setText("Select a game to launch")
    ...
    def select_game(self, card: GameCard):
        ...
        self.status_label.setText(f"Ready: {card.game_name}")
    ...
    def on_launch_clicked(self):
        ...
        if not rel_exe_path:
            self.status_label.setText("Error: Empty executable path!")
            return
        ...
        if not os.path.exists(source_mfs):
            self.status_label.setText(
                "Error: Missing Binaries/Win64/main.mfs template!"
            )
            self.status_label.setStyleSheet("color: #ff5555; font-size: 12px;")
            print(f"[Launcher] Template missing at: {source_mfs}")
            return
        ...
        try:
            ...
            print(
                f"[Launcher] Created replica from {source_mfs} -> {repr(target_exe)}"
            )

            subprocess.Popen([target_exe], cwd=target_dir)

            self.status_label.setText(f"Running Replica: {game_name}")
            ...
            self.status_label.setStyleSheet("color: #ff5555; font-size: 12px;")
            print(f"[Launcher] Execution error: {e}")
